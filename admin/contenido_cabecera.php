...
if (!$usuario->compruebaDerecho("GCON"))
{
        ...
        <section class="content">
        	<div class="row">
        		<div class="col-xs-10" id="">
            		<?php
            		      $apartado = 'cabecera';
            		      $idioma_defecto = idioma_por_defecto($conn);
            		
            		      $str_respuesta = '<table class="table table-bordered table-striped">';
            		      $str_respuesta .= '<tr><th></th><th>Etiqueta</th><th></th><th></th><th></th><th></th></tr>';
            		      $query = sprintf("SELECT * FROM texto WHERE apartado = '%s' AND idioma = '%s'", $apartado, $idioma_defecto);
            		      $result = mysqli_query($conn, $query);
            		      while($row = $result->fetch_assoc())
            		      {
            		          $str_respuesta .= '<tr><td></td><td>'.$row['etiqueta'].'</td><td>'.$row['texto'].'</td></tr>';
            		          $query = sprintf("SELECT * FROM texto WHERE etiqueta = '%s' AND idioma <> '%s'", $row['etiqueta'], $idioma_defecto);
            		          $result_idioma = mysqli_query($conn, $query);
            		          while ($fila_idioma = $result_idioma->fetch_assoc())
            		          {
            		              $str_respuesta .= '<tr><td></td><td></td><td>'.$fila_idioma['texto'].'</td></tr>';
            		          }
            		      }
            		      $str_respuesta .= '</table>';
            		      echo $str_respuesta;
